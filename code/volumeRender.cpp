...
void* compress(FILE* fp, FILE* fout, int nu, int rest, int div,char* path, char* file_out, size_t *size)
{
	*size = volumeSize.width*volumeSize.height*div*sizeof(VolumeType);
    void *h_volume = loadRawFile(fp,path, *size);
	VolumeType *d2 = (VolumeType*)h_volume;
	
	int i,j,k,l=0;
	size_t v1=0,w1=0,sz=0,skip=0,sval=0;
	printf("\nnu = %d rest = %d\n",nu,rest);
	sval=skip = volumeSize.width*volumeSize.height*div;
	if(nu>0)			
	{
		for(i=0; i<div*volumeSize.width*volumeSize.height; i+=2)	//skip col
		{		//printf("col %d\n",k);
			if(i%1802240==0)
				i+=(1802240);													//skip frame
		//	fprintf(fout,"%u ",d2[i]);
			fwrite (&(d2[i]) , sizeof(VolumeType), 1, fout);			//write in binary file
		}
	
		v1 = (div)/2;			
		w1 = volumeSize.width/2;
		sz = w1*volumeSize.height*v1;		
	...
	for(int it = 1; it<nu; it++)
	{
		...
		free(d2);
	    h_volume = loadRawFile(fp,path, *size);
	//	printf("loaded");
		d2 = (VolumeType*)h_volume;
		for(i=0; i<div*volumeSize.width*volumeSize.height; i+=2)	//skip col
		{
			if(i%1802240==0)
				i+=(1802240);													//skip frame
			fwrite(&(d2[i]) , sizeof(VolumeType), 1, fout);
		}
		sval+=skip;
	...
	{
		free(d2);
		*size = volumeSize.width*volumeSize.height*rest*sizeof(VolumeType);
	    h_volume = loadRawFile(fp,path, *size);
	//	printf("loaded");
		d2 = (VolumeType*)h_volume;
		for(i=0; i<rest*volumeSize.width*volumeSize.height; i+=2)	//skip col
		{		//printf("col %d\n",k);
			if(i%1802240==0)
				i+=(1802240);													//skip frame
		...
		sval+=(rest*volumeSize.width*volumeSize.height);
		sz+=w1*volumeSize.height*rest;		
		v1 += ((rest/2));
	}
	...
	fclose(fp);
	free(d2);
	FILE *fp2 = fopen(file_out, "rb");
	volumeSize.width/=2;
	fseek(fp2, 0,SEEK_SET);
	
	volumeSize.depth = v1;
	printf(" %d %d %d",volumeSize.depth,volumeSize.width,volumeSize.height);
	size_t num = volumeSize.depth*volumeSize.width*volumeSize.height;
	*size = num*sizeof(VolumeType);

	void *data = malloc(*size);
    size_t read = fread(data, sizeof(VolumeType), num, fp2);				//size of compressed file
	fclose(fp2);
	return data;
}
